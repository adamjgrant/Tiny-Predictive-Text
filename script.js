...
const find_a_suggestion = (word_parts) => {
  ...
  // Join the word parts into a single string, then split by "."
  let partsAfterLastPeriod = word_parts.join(" ").split(".").pop().trim();

  // Split the content after the last period by spaces to get the final word parts
  word_parts = partsAfterLastPeriod.split(" ").filter(word => word);
